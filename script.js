...
function safeMapLink(record) {
  const savedLink = String(record.googleMapsLink || "").trim();
  if (savedLink) {
    try {
      ...
      if (url.protocol === "https:" && isGoogleMapsHost) return url.href;
    } catch {
      // Fall back to valid coordinates when the saved map link is malformed.
    }
  }

  const latitudeText = String(record.latitude || "").trim();
  const longitudeText = String(record.longitude || "").trim();
